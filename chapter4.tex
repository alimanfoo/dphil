...
%
These analyses gave conflicting results depending on which chromosome arm was analysed, with 3R indicating that Kenya was sister with \textit{An. coluzzii} ($D(KE, BFcol; BFgam, chri); Z=0.1$), 3L indicating Kenya was a cryptic taxon ancestral to both species ($D(BFgam, BFcol; KE, chri); Z=-0.7$) and the X chromosome not providing clear support for any one hypothesis.
%
...
%
All $f_4$ tests with \textit{An. coluzzii} from Angola and Burkina Faso as sister populations provided significant evidence for admixture between Angola and \textit{An. gambiae}, with the strongest signal obtained for Gabon (e.g., $D(AOcol, BFcol; GAgam, chri); Z=14.0$).
%
Conversely, all tests with two \textit{An. gambiae} populations as sisters provided evidence for admixture between Gabon \textit{An. gambiae} and \textit{An. coluzzii}, with the strongest signal obtained for Angola (e.g., $D(GAgam, GNgam; AOcol, chri); Z=11.1$).
%
...
%
I used $f_4$ tests of the form $D(A, B; arab, chri)$ where $A$ and $B$ were all pairs of Ag1000G phase 1 populations to investigate whether gene flow with arabiensis is greater for some populations relative to others.
%
These tests were significant in some cases (e.g., $D(BFgam, BFcol; arab, chri); Z=8.0$; $D(UGgam, BFgam; arab, chri); Z=6.9$) but not others, allowing for a partial ordering of populations by relative degree of introgression .
%
This partial ordering was $GW < \{AOcol, BFcol\} < \{GNgam, BFgam, CMgam, GAgam\} < UGgam < KE$, indicating that introgression with \textit{An. arabiensis} is higher in \textit{An. gambiae} relative to \textit{An. coluzzii}, and higher in easterly populations relative to westerly populations.
